...
func dumpGoroutineStacks() {
	...
	if n == 0 {
		_, _ = fmt.Fprintln(os.Stderr, "No goroutine stack traces available")
		return
	}

	_, _ = fmt.Fprintln(os.Stderr, "========== Goroutine Stack Traces ==========")
	_, _ = fmt.Fprintln(os.Stderr)
	if _, err := os.Stderr.Write(buf[:n]); err != nil {
		_, _ = fmt.Fprintf(os.Stderr, "Error writing stack traces: %v\n", err)
	}
	_, _ = fmt.Fprintln(os.Stderr)
	_, _ = fmt.Fprintln(os.Stderr, "========== End of Stack Traces ==========")
}
